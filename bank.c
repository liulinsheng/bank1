...
int main()//主函数
{
	...
	while (1)
	{
		...
		switch (x)
		{
		case 1:  {system("clear");  	char a[] = "123123";
			if (Mimayincan(a) == 1)
				MainMeun1(L, L1);
			exit(0); }                   //调用管理员主菜单函数
		case 2:  {system("clear");  	Position p = Logon(L);
			if (p)
...
List MakeEmpty(List L)//空链表创建函数
{
	...
	if (!L)                                     //相当于 if(L==NULL) ,即指针L指向的空间为空
	{
		...
		char buf[1024]; fgets(buf,1024,stdin); system("clear");                       //暂停屏幕显示
		return NULL;                           //返回上一级菜单
	}
...
void TravelList(List L)//遍历链表函数
{
	...
	printf("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - -\n");
	char buf[1024]; fgets(buf,1024,stdin); system("clear");
}
...
while (1)
	{
		...
		switch (x)
		{
		case 1:  system("clear");  Add(L);    system("clear");  MainMeun1(L, L1);          //注册账户
		case 2:  system("clear");  Delete(L, L1);    system("clear");  MainMeun1(L, L1);    //注销账户
		case 3:  system("clear");  ViewMeun(L, L1);      system("clear");  MainMeun1(L, L1);                          //查询客户信息
		case 4:  system("clear");  RecordMeun(L1);      return;                         //查询客户存取款信息
		case 5:  system("clear");  ModMeun(L, L1);         return;                       //修改客户信息
		case 6:  system("clear");  OrderMeun(L, L1);     return;                         //信息查看
		case 7:  {system("clear"); 
			Position p = Logon(L); MainMeun2(p->num, L, L1); MainMeun1(L, L1);} break;     //客户存取款,修改密码等操作
		case 8:  system("clear");  printf("是否需要重置系统，回复'Y'/'N'\n");
			if (getchar() == 'Y')
			{
				MakeFileEmpty(), MakeFileEmpty1(); system("clear"); return;
			}//重置系统
			MainMeun1(L, L1);
		case 9:  system("clear");  jiesuo(L, L1); MainMeun1(L, L1); break;                     //返回上一层
		case 0:  system("clear");  main();   break;                     //返回上一层
		}
		return;
	}
}
void MainMeun2(char num[], List L, List1 L1)//客户主菜单
{
	...
	while (1)
	{
		...
		switch (x)
		{
		case 1:  system("clear");  Add1(sj, p, L, L1);   system("clear"); MainMeun2(p->num, L, L1);                //存款
		case 2:  system("clear");  if (p1->next == NULL)
		{
			printf("\n无操作记录： ");
			char buf[1024]; fgets(buf,1024,stdin); system("clear");
			return;
		}
				 ViewNum1(num, L1);  system("clear"); MainMeun2(p->num, L, L1);                //操作记录查询
		case 3:  system("clear");  ViewNum(num, L);    system("clear"); MainMeun2(p->num, L, L1);               //个人信息查询
		case 4:  system("clear");  ModPassword(p, L);  system("clear"); MainMeun2(p->num, L, L1);              //修改密码
		case 5:  system("clear");  main();  break;                          //返回上一层
		}
		return;
	}
}
void OrderMeun(List L, List1 L1)//信息查看菜单
{
	...
	while (1)
	{
		...
		switch (x)
		{
		case 1:  system("clear");  L = SortNum(L);      system("clear");  OrderMeun(L, L1);              //按账号排序
		case 2:  system("clear");  L = SortMoney(L);    system("clear");  OrderMeun(L, L1);           //按余额排序
		case 3:  system("clear");  L1 = SortNum1(L1);   system("clear");  OrderMeun(L, L1);                 //按账号排序
		case 4:  system("clear");  L1 = SortTime1(L1);  system("clear");  OrderMeun(L, L1);                //按时间排序
		case 5:  system("clear");  MainMeun1(L, L1);                                    //返回上一层
		}
	}
}
void ViewMeun(List L, List1 L1)//查询客户信息菜单
{
	...
	while (1)
	{
		...
		switch (x)
		{
		case 1:  system("clear");  char num[30];getchar(); printf("\n请输入所要查询的账号:  ");
			gets(num); ViewNum(num, L);      system("clear");  ViewMeun(L, L1);   break;   //按账号查询
		case 2:  system("clear");  char ID[20];getchar(); printf("\n请输入需要查询的身份证： ");
			gets(ID); ViewID(ID, L);         system("clear");  ViewMeun(L, L1);   break;   //按身份证号查询
		case 3:  system("clear");  char name[10];getchar(); printf("\n请输入需要查询的姓名： ");
			gets(name); ViewName(name, L);  system("clear");  ViewMeun(L, L1);    break;  //按姓名查询
		case 4:  system("clear");  char tel[20];getchar(); printf("\n请输入需要查询的电话： ");
			gets(tel); ViewTel(tel, L);      system("clear");  ViewMeun(L, L1);   break;   //按电话查询
		case 5:  system("clear");  ViewCom(L);  MainMeun1(L, L1);    break;                   //按电话和姓名查询
		case 6:  system("clear");  MainMeun1(L, L1); break;                   //返回上一层
		}
	}
}
void RecordMeun(List1 L1)//查询客户存取款记录菜单
{
	...
	while (1)
	{
		...
		switch (x)
		{
		case 1:  system("clear");getchar();  printf("\n请输入所要查询的账号:  "); gets(num); ViewNum1(num, L1);
			system("clear");  RecordMeun(L1);
		case 2:  system("clear");getchar();  printf("\n请输入所要查询的姓名:  "); gets(name); ViewName1(name, L1);
			system("clear");  RecordMeun(L1);
		case 3:  system("clear");  ViewCom1(L1);  MainMeun1(L, L1);    break;                   //按账号和操作时间查询
		case 4:  system("clear");  MainMeun1(L, L1);                                    //返回上一层
		}
	}
}
void ModMeun(List L, List1 L1)//修改菜单
		...
		case 1:  system("clear");  printf("\n请输入查询修改的账号: ");     gets(num);      //按账号查询
			while (p)  //判断P的Next指针是否为空，且P的下一个结点的客户电话是否为查找账号
			{
				if (strcmp(num, p->num) == 0)
				{
					...
					{
						printf("未找到您想要修改的账号~\n");
						char buf[1024]; fgets(buf,1024,stdin); system("clear"); system("clear");  ModMeun(L, L1);
					}
				}
				p = p->next;
				if (!p)
			...
			system("clear");  ModMeun(L, L1);
		case 2:  system("clear");getchar();  printf("\n请输入查询修改的姓名: ");     gets(name);      //按姓名查询
			CircleName(name, p, L, L1);                system("clear");  ModMeun(L, L1);
		case 3:  system("clear");getchar();  printf("\n请输入查询修改的电话: ");     gets(tel);       //按电话查询
			CircleTel(tel, p, L, L1);                  system("clear");  ModMeun(L, L1);
		case 4:  system("clear");getchar();  printf("\n请输入查询修改的身份证号: "); gets(ID);        //按身份证查询
			CircleID(ID, p, L, L1);                   system("clear");  ModMeun(L, L1);
		case 5:  system("clear");  MainMeun1(L, L1);                             //返回上一层
		}
	}
}
void ModMeun1(Position p, List L, List1 L1)//修改客户信息菜单
{
	...
	while (1)
	{
		...
		switch (x)
		{
		case 1:  system("clear");  ModName(p, L, L1);   system("clear");  ModMeun1(p, L, L1);                //修改姓名
		case 2:  system("clear");  ModPassword(p, L);  system("clear");  ModMeun1(p, L, L1);                //修改密码
		case 3:  system("clear");  ModTel(p, L);       system("clear");  ModMeun1(p, L, L1);                //修改电话
		case 4:  system("clear");  ModMeun(L, L1);                            //返回上一层
		}
	}
}
...
void Add1(char sj[], Position p, List L, List1 L1)//收支记录
{
	...
	if (p->type == 1)//储蓄卡
	{
		...
		{
			printf("\n余额不足！");
			char buf[1024]; fgets(buf,1024,stdin); system("clear"); return;
		}
...
void jiesuo(List L, List1 L1)
{
	...
	while (1)
	{
		...
		case 2:  printf("\n\t\t请输入您需要挂失的账号：");
			...
			printf("\t\t该账号完成挂失\n");
			char buf[1024]; fgets(buf,1024,stdin); system("clear");MainMeun1(L, L1); break;
		case 3:  printf("\n\t\t请输入您需要取消挂失的账号：");
			gets(num);
			while (p != NULL && strcmp(num, p->num) != 0)
				p = p->next;
